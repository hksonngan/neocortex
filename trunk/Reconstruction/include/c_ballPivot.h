...
public:
	...
	ballPivot(ScanData *InputData, int MeshStep_X, int MeshStep_Y, int MeshStep_Z, GLfloat MinVoxelDensity);													// из сохранЄнного набора данных
	ballPivot(ScanData *InputData, TVoxelSegments *VoxelSegments, int SegmentIndex, int MeshStep_X, int MeshStep_Y, int MeshStep_Z, GLfloat MinVoxelDensity);	// из 3D-сегмента	
	~ballPivot();
	void buildMesh();
	void renderMesh();
};
...
int ballPivot::if_point_on_ball(c_edge &edge, c_point &center, list <c_point> &points)
{
	...
	for(list <c_point>::iterator point_iter=points.begin(); point_iter!=points.end(); point_iter++)
	{
		...
		// ≈сли рассто€ние между точкой и центром сферы равно радиусу то точка лежит на поверхности сферы
		...
		if ((cur_dist<radius+flt)&&(cur_dist>radius-flt))
		{
			...
			if(scalar>0)		//лева€ точка
			{
				if(edge.is_lp_exists) continue;
			}
			else if(scalar<0)	//права€ точка
			{
...
{
	float a=0;			//”гол поворота
	int point_num=-1;
	int direct=1;		//Ќаправление поворота шара. 1-по часовой
	...
	getPointsSection(points, 0, COUNT_OF_ALL_SHOWED_POINTS-1, all_points[edge.getFP()], 2*radius, my_points);	//Ќаходим точки в пределах 2*–адиус от текущего ребра
	...
	//сохран€ем начальное положение центра
	null_center=center;
	...
	//—охран€ем текущее положение центра
...
ballPivot::ballPivot(ScanData* InputData, int MeshStep_X, int MeshStep_Y, int MeshStep_Z, GLfloat MinVoxelDensity)
{
 ...
 // ѕодсчитываем количество видимых вокселов
 ...
 // »нициализируем значени€ всех видимых вокселов
 int num=0;
 for (int i = 0; i < InputData->sizeX; i+=STEP_X)
 for (int j = 0; j < InputData->sizeY; j+=STEP_Y)
 for (int k = 0; k < InputData->sizeZ; k+=STEP_Z)
...
ballPivot::ballPivot(ScanData *InputData, TVoxelSegments *VoxelSegments, int SegmentIndex, int MeshStep_X, int MeshStep_Y, int MeshStep_Z, GLfloat MinVoxelDensity)
{
 MIN_DENSITY = MinVoxelDensity;
 STEP_X = MeshStep_X; STEP_Y = MeshStep_Y; STEP_Z = MeshStep_Z;
 razm_x=InputData->scaleX; razm_y=InputData->scaleY; razm_z=InputData->scaleZ;
